...
export async function articleRoutes(fastify) {
    ...
    fastify.get('/api/articles', async (request, reply) => {
        const { page = 1, limit = 10, tag, category } = request.query;
    ...
    fastify.put('/api/articles/:id', { onRequest: [authMiddleware] }, async (request, reply) => {
        ...
        const body = request.body;
        const article = await Article.findByIdAndUpdate(id, { ...body, updatedAt: new Date() }, { new: true, runValidators: true });
        if (!article) {
            return reply.status(404).send({ error: '文章未找到' });
        }
        return article;
    ...
    fastify.get('/api/articles/stats/heatmap', async () => {
        ...
        const articles = await Article.find({ published: true, createdAt: { $gte: oneYearAgo } }, { createdAt: 1 });
        const heatmap = {};
        articles.forEach((a) => {
